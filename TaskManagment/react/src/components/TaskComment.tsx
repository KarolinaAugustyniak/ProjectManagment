...
const TaskComment = ({ comment }) => {
  ...
  const formattedDate = new Date(commentDate).toLocaleString(undefined, options);

  return (
    <li className="task-comment">
      <div className="task-comment__wrapper">
        <User username={commentedByUser.username} image={commentedByUser.profileImageFileName}/>
